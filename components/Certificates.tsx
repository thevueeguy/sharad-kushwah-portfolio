...
export default function Certificates({ certificates }: Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 1 }} className="relative h-screen flex overflow-hidden flex-col text-left max-w-full justify-evenly items-center py-10">
      <div className="flex flex-col uppercase gap-3">
        <h3 className="tracking-[10px] md:tracking-[20px] text-center text-gray-500 text-base sm:text-xl md:text-2xl mx-auto font-medium">&nbsp;Certificates</h3>
        <h3 className="text-center tracking-[3px] text-gray-500 text-xs sm:text-sm md:text-md">Click on a certificate to view in full size</h3>
      </div>
            ...
            <SwiperSlide key={certificate._id} virtualIndex={index} className="!h-1/3 my-auto align-middle !flex !mx-auto">
              <div className={"flex flex-col items-center hover:scale-110 transition-all duration-200 cursor-pointer group opacity-80 hover:opacity-100 " + (index & 1 ? "justify-start" : "justify-end")}>
                <motion.img
                  onClick={() => {
                    setIsClicked(true);
                    setUrl(urlFor(certificate?.certificate).url());
                  }}
                  initial={{ y: -50, opacity: 0 }}
                  whileInView={{ y: 0, opacity: 1 }}
                  transition={{ duration: 1 }}
                  className="relative w-24 h-24 sm:w-44 sm:h-44 xl:w-60 xl:h-60 mx-auto object-contain pt-0"
                  data-tooltip-id="tooltip"
                  data-tooltip-content={certificate?.summary}
                  data-tooltip-place="bottom"
                  src={urlFor(certificate?.certificate).url()}
                  alt="profile-photo"
                />
              </div>
            </SwiperSlide>
