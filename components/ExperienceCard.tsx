    ...
    >
      <article className="rounded-lg w-full snap-center cursor-pointer transition-opacity duration-200">
        <div className="px-0 md:px-5 flex flex-col justify-center items-center text-center space-y-1 md:space-y-7">
          <h4 className="text-base sm:text-xl md:text-2xl font-light">{experience?.jobTitle}</h4>
          <div className="text-base sm:text-lg md:text-xl font-bold py-1">{experience?.company}</div>
          <div className="w-1/2 flex py-2 justify-center gap-2 flex-wrap">
            {experience?.technologies.map((technology) => (
              <motion.img key={technology._id} className="h-6 w-6 md:h-8 md:w-8 lg:h-10 lg:w-10 object-fill rounded-full bg-white border-2 border-white-800 border-double" src={urlFor(technology.image).url()} alt={technology.title} />
            ))}
          </div>
        ...
        <motion.div className="flex flex-row gap-3 justify-center items-center align-middle w-full h-full mt-3" onClick={() => setIsOpened(!isOpened)}>
          <span>Read {isOpened ? "Less" : "More"}</span>
          {isOpened ? <ChevronUpIcon className="text-red-800 h-5 w-5 md:h-6 md:w-6 lg:h-7 lg:w-7 animate-bounce" /> : <ChevronDownIcon className="text-red-800 h-5 w-5 md:h-6 md:w-6 lg:h-7 lg:w-7 animate-bounce" />}
        </motion.div>
      </article>
    </VerticalTimelineElement>
  );
