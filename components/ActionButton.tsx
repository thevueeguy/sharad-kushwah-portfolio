...
export default function ActionButton() {
  ...
  return (
    <div className="fixed bottom-0 right-0 p-5 flex items-end justify-end w-18 h-18 md:w-24 md:h-24 z-50">
      ...
      <div data-tooltip-id="tooltip" data-tooltip-content="Download Resume" data-tooltip-place="left" className={`absolute cursor-pointer active:scale-110 rounded-full transition-all duration-100 ease-out scale-y-0 flex  p-2 hover:p-3 bg-green-300 scale-100 hover:bg-slate-600 hover:text-white text-black ${click ? "scale-y-100 -translate-x-16" : ""}`}>
        <a href="Sharad_Resume.pdf" download>
          <ArrowDownTrayIcon className="h-3 w-3 md:h-5 md:w-5 m-1" />
        </a>
      </div>
      <div data-tooltip-id="tooltip" data-tooltip-content="Source Code" data-tooltip-place="left" className={`absolute cursor-pointer active:scale-110 rounded-full transition-all duration-100 ease-out scale-x-0 flex  p-2 hover:p-3 bg-blue-300 hover:bg-slate-600 hover:text-white text-black ${click ? "scale-x-100 -translate-y-16" : ""}`}>
