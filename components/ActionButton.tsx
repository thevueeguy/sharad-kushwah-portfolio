...
export default function ActionButton() {
  ...
  return (
    <div className="fixed bottom-0 right-0 p-5 flex items-end justify-end w-18 h-18 md:w-24 md:h-24 z-20000">
      <Tooltip id="tooltip" />
      <div
        className={`text-white shadow-xl z-20000 ease-in-out duration-100 flex items-center justify-center p-3 rounded-full bg-gradient-to-r from-red-400 to-red-700 z-50 absolute ${click ? "rotate-45" : ""}`}
        onClick={(e) => {
          e.preventDefault();
          setClick(!click);
        }}
      ...
      <div data-tooltip-id="tooltip" data-tooltip-content="Source Code" data-tooltip-place="left" className={`absolute z-20000 rounded-full transition-all duration-100 ease-out scale-x-0 flex  p-2 hover:p-3 bg-blue-300 hover:bg-slate-600 hover:text-white text-black ${click ? "scale-x-100 -translate-y-16" : ""}`}>
