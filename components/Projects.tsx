            ...
            <SwiperSlide key={project._createdAt} virtualIndex={index}>
              <div className="w-full flex justify-center items-center">
                <div key={project?._id} className="w-screen flex-shrink-0 snap-center flex flex-col space-y-5 justify-center items-center p-20 md:p-44 h-screen">
                  <motion.img initial={{ y: -200, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ duration: 1 }} className="relative rounded-full h-24 w-24 sm:h-32 sm:w-32 xl:h-36 xl:w-36 mx-auto object-cover pt-0" src={urlFor(project.image).url()} alt="profile-photo" />
                  <div className="space-y-10 px-0 md:px-10 max-w-6xl">
                    <h4 className="text-base sm:text-xl md:text-2xl font-semibold text-center">{project?.title}</h4>

                    <div className="flex flex-row space-x-3 lg:space-x-10 items-center justify-center ">
                      {project?.technologies.map((technology) => (
                        <motion.img className="h-8 w-8 lg:h-16 lg:w-16 rounded-full bg-white object-contain border-2 lg:border-4 border-sky-600" key={technology._id} src={urlFor(technology.image).url()} alt="" />
                      ))}
                    </div>

                    <p className="text-[#e9e5e5] text-center text-xs sm:text-base md:text-base lg:text-left ">{project?.summary}</p>
                  </div>
                </div>
              </div>
            </SwiperSlide>
